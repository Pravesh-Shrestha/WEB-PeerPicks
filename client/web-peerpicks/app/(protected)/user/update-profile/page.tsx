...
import { 
  ...
  User as UserIcon, Calendar, Phone, 
  Fingerprint, Sparkles, Upload, 
  ...
  return (
    <div className="min-h-screen bg-[#0B0C10] text-white selection:bg-[#D4FF33] selection:text-black">
      ...
      <nav className="p-6 flex items-center justify-between border-b border-white/10 backdrop-blur-xl sticky top-0 z-50">
        <button onClick={() => router.back()} className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 hover:text-[#D4FF33] transition-all">
          <ChevronLeft size={16} />
          <span>Discard</span>
        </button>
        <div className="flex items-center gap-2 px-4 py-2 bg-white/5 rounded-2xl border border-white/10">
          <Fingerprint size={14} className="text-[#D4FF33]" />
          <span className="text-[9px] font-black uppercase tracking-widest text-white/60">IDENTITY_UPDATE</span>
        </div>
      </nav>
      ...
      <main className="max-w-xl mx-auto px-6 py-12">
        <header className="mb-10 text-center">
          <h1 className="text-4xl font-black italic uppercase tracking-tighter mb-2">
            Profile <span className="bg-gradient-to-r from-[#D4FF33] to-[#5D44F8] bg-clip-text text-transparent">Update</span>
          </h1>
          <p className="text-gray-500 text-[9px] font-black uppercase tracking-[0.3em]">Configure your network presence</p>
        </header>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="flex flex-col items-center">
            ...
            <span className="mt-4 text-[9px] font-black uppercase tracking-widest text-gray-600">Change Visual Identity</span>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* FULL NAME */}
            <InputField label="Full Name" icon={UserIcon} value={fullName} onChange={(e: any) => setFullName(e.target.value)} />

            {/* PHONE */}
            <InputField label="Contact Phone" icon={Phone} value={phone} onChange={(e: any) => setPhone(e.target.value)} />

            {/* DOB */}
            <InputField label="Birth Date" icon={Calendar} type="date" value={dob} onChange={(e: any) => setDob(e.target.value)} />
                ...
                {(['male', 'female'] as const).map((g) => (
                  <button
                    ...
                    className={`flex-1 py-3.5 rounded-xl border text-[10px] font-black uppercase tracking-widest transition-all ${
                      gender === g ? 'bg-[#D4FF33] border-[#D4FF33] text-black' : 'bg-white/5 border-white/10 text-gray-500 hover:border-white/20'
                    }`}
                  >
                    {g}
                  </button>
          ...
          <button type="submit" disabled={isSubmitting} className="w-full py-5 bg-[#D4FF33] text-black rounded-2xl font-black uppercase text-[11px] tracking-[0.4em] flex items-center justify-center gap-3 shadow-2xl hover:shadow-[#D4FF33]/40 transition-all disabled:opacity-50">
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="relative group">
        ...
        <input {...props} className="w-full bg-white/5 border border-white/10 focus:border-[#D4FF33]/50 outline-none rounded-xl py-3.5 pl-12 pr-4 text-[11px] font-bold transition-all" />
      </div>
    </div>
  );
}