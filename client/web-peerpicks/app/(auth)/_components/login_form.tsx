...
export default function LoginForm() {
  ...
  const onSubmit = async (data: LoginData) => {
    ...
    try {
      ...
      if (result?.success) {
        setIsSuccess(true);
        setMessage("Teleporting to dashboard...");
      ...
      setIsSuccess(false);
      setMessage("SYSTEM ERROR: UNABLE TO REACH AUTH NODE");
    }
  };
  ...
  return (
    <div className="w-full max-w-md mx-auto">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        <fieldset disabled={isSubmitting || isSuccess} className="space-y-5">
          <div className="space-y-1">
            <div
              className={`bg-white rounded-xl flex items-center px-4 h-[48px] border-2 transition-all ${errors.email ? "border-red-500" : "border-transparent focus-within:border-indigo-500"}`}
            >
              <input
                {...register("email")}
          ...
          <div className="space-y-1">
            <div
              className={`bg-white rounded-xl flex items-center px-4 h-[48px] border-2 transition-all ${errors.password ? "border-red-500" : "border-transparent focus-within:border-indigo-500"}`}
            >
              <input
                {...register("password")}
      ...
      <div className="mt-12 pt-8 border-t border-white/5 flex flex-col items-center">
        <p className="text-slate-500 text-[9px] uppercase font-bold tracking-[0.2em] mb-5">
          Quick login with
        </p>
